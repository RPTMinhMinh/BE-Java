...
public class Company {
  ...
  o	Xóa: Cho phép xóa nhân viên hoặc trưởng phòng khỏi hệ thống.
    ...
    public void tongLuongNhanVien(){
        ...
        for (Employee employee:this.danhSachNhanVien){
            double luong = employee.cachTinhLuongThang();
            tongLuongNhanVien += luong;
            System.out.println("Nhan vien co ma "+employee.getMaMV()+" co tong luong trong thang la: " +luong);
        }
        System.out.println("--------------------");
        System.out.println("Tong luong cua toan bo nhan vien la: " + tongLuongNhanVien);
        System.out.println("--------------------");
    }

    public void nhanVienCoLuongCaoNhat(){
        Staff nhanVienCoLuongCaoNhat = null;
        ...
        for (Employee employee:this.danhSachNhanVien){
            ...
            if (luong > luongCaoNhat){
                luongCaoNhat = luong;
                nhanVienCoLuongCaoNhat = (Staff) employee;
            }
        }
        System.out.println("------------------------");
        System.out.println("Thong tin nhan vien co luong cao nhat: ");
        nhanVienCoLuongCaoNhat.hienThiThongTin();
        System.out.println("Luong 1 thang " + luongCaoNhat);
        System.out.println("-----------------------");
    }

    public void truongPhongCoNVMax(){
        Manager truongPhongCoNVMax = null;
        int soNhanVienMax = 0;
        for (Employee employee:this.danhSachNhanVien){
            if (employee instanceof Staff || employee instanceof Director){
                continue;
            }
            Manager manager = (Manager) employee;
            int soNhanVien = manager.getDanhSachNhanVien().size();
            if (soNhanVien > soNhanVienMax){
                soNhanVienMax = soNhanVien;
                truongPhongCoNVMax = manager;
            }
        }
        System.out.println("------------------------");
        System.out.println("Thong tin quan ly co nhieu nhan vien nhat: ");
        truongPhongCoNVMax.hienThiThongTin();
        System.out.println("So nhan vien quan ly: " + soNhanVienMax);
        System.out.println("-----------------------");
    }

    public void luongGiamDan(){
        this.danhSachNhanVien.sort( new Comparator<Employee>(){
            @Override
            public int compare(Employee o1, Employee o2) {
                double low = o1.cachTinhLuongThang();
                double high = o2.cachTinhLuongThang();
                return Double.compare(high, low);
            }
        });
        System.out.println("Danh sach luong toan bo nhan vien giam dan: ");
        for (Employee employee:this.danhSachNhanVien){
            employee.hienThiThongTin();
            System.out.println("luong 1 thang " + employee.cachTinhLuongThang());
        }

    }

    public void giamDocCoCoPhanMax(){
        Director giamDocCoCoPhanMax = null;
        Double coPhanMax = 0.0;
        for (Employee employee:this.danhSachNhanVien){
            if (employee instanceof Manager || employee instanceof Staff){
                continue;
            }
            Director doc = (Director) employee;
            if(doc.getCoPhan() > coPhanMax){
                coPhanMax = doc.getCoPhan();
                giamDocCoCoPhanMax = (Director) employee;
            }
        }
        System.out.println("------------------------");
        System.out.println("Thong tin giam doc co co phan cao nhat: ");
        giamDocCoCoPhanMax.hienThiThongTin();
        System.out.println("Co phan: " + coPhanMax);
        System.out.println("-----------------------");
